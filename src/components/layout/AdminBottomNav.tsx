...
const adminNavItems: NavItem[] = [
  { href: "/profile/admin", label: "Dash", icon: LayoutDashboard },
  { href: "/profile/admin/products", label: "Products", icon: Package },
  { href: "/profile/admin/categories", label: "Cats", icon: FolderTree },
  ...
  { href: "/profile/admin/item-requests", label: "Reqs", icon: FileQuestion },
...
export function AdminBottomNav() {
  ...
  return (
    <nav
      className="md:hidden fixed bottom-0 left-0 right-0 z-50 border-t bg-white overflow-hidden"
      style={{
    ...
    >
      <div className="pb-[env(safe-area-inset-bottom)]">
        ...
        <div className="grid grid-cols-5 h-[60px] w-full max-w-[500px] mx-auto">
          ...
          >
            <ArrowLeft className="h-[18px] w-[18px] text-[#2D5A27]" />
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              <button className="flex flex-col items-center justify-center h-full tap-active py-1">
                <Menu className="h-[18px] w-[18px] text-[#6B7280]" />
                <span className="text-[9px] mt-0.5 font-medium text-[#6B7280]">More</span>
              </button>
            </SheetTrigger>
            <SheetContent
              side="bottom"
